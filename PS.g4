...
EQUALITY_CMD: (VARIABLE | LETTER | GREEK_LETTER | DIGIT )+ (EQUAL | LT | LTE | GT | GTE | UNEQUAL) (VARIABLE | LETTER | GREEK_LETTER | DIGIT | FUNC_LCM)+;


LOGICAL: (AND | OR ) L_BRACE ( VARIABLE | DIGIT | LETTER | EQUALITY_CMD | AND | COMMA | (LEN_CMD GT DIGIT)  )+ R_BRACE;

NOT_CMD: NOT L_BRACE (VARIABLE|LETTER) R_BRACE;


FOR_CMD: VARIABLE FOR VARIABLE;

MULTIPLE: (AND | OR | NOT | VARIABLE) L_BRACE (AND | OR | NOT | L_BRACE | R_BRACE | VARIABLE | COMMA)+ R_BRACE;
...
atom: (LETTER_NO_E | GREEK_LETTER | accent) subexpr? | SYMBOL | NUMBER | PERCENT_NUMBER | E_NOTATION | DIFFERENTIAL | mathit | VARIABLE | EQUALITY_CMD | INTERVAL | ABSOLUTE | NRT | LOGICAL | SUMMATION | EQUIVALENCE  | SET_CMD | FOR_CMD | LEN_CMD | NOT_CMD | MULTIPLE;
